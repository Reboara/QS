...
// Функция для нахождения максимальной длины строки в списке
...
ostream &operator<<(ostream &os, const Statistics &stats)
{
    size_t maxQueue1Length = maxStringLength({ "1. Transactions at peak:    " + to_string(stats.QMAX[0]),
                                               ...
                                               "5. Average transactions:     " + to_string(stats.QAVECONT[0]),
                                               "6. Average time in queue:    " + to_string(stats.QAVETIME[0]),
                                               "7. Average time (-0):        " + to_string(stats.QAVETIME0[0]) });
    ...
    size_t maxQueue2Length = maxStringLength({ "1. Transactions at peak:    " + to_string(stats.QMAX[1]),
                                               ...
                                               "5. Average transactions:     " + to_string(stats.QAVECONT[1]),
                                               "6. Average time in queue:    " + to_string(stats.QAVETIME[1]),
                                               "7. Average time (-0):        " + to_string(stats.QAVETIME0[1]) });
    
     size_t maxFacultyStatsLength = maxStringLength({ "Entered transactions:      " + to_string(stats.WENTRY[0]),
                                                      "Workload, %:               " + to_string(stats.WLOAD[0]),
                                                      "Average service time:      " + to_string(stats.WAVETIME[0]) });
    ...
    os << "* 5. Average transactions:    " << setw(9) << stats.QAVECONT[0] << " *" << endl;
    os << "* 6. Average time in queue:   " << setw(9) << stats.QAVETIME[0] << " *" << endl;
    os << "* 7. Average time (-0):       " << setw(9) << stats.QAVETIME0[0] << " *" << endl;
    os << string(maxQueue1Length + 4, '*') << endl;
    os << endl;
    ...
    os << "* 5. Average transactions:    " << setw(9) << stats.QAVECONT[1] << " *" << endl;
    os << "* 6. Average time in queue:   " << setw(9) << stats.QAVETIME[1] << " *" << endl;
    os << "* 7. Average time (-0):       " << setw(9) << stats.QAVETIME0[1] << " *" << endl;
    os << string(maxQueue2Length + 4, '*') << endl;
    os << endl;
    ...
    os << "* Workload, %:                " << setw(8) << stats.WLOAD[0] << " *" << endl;
    os << "* Average service time:       " << setw(8) << stats.WAVETIME[0] << " *" << endl;
    os << string(maxFacultyStatsLength + 4, '*') << endl;
    os << endl;
...
void System::simulate(double _timeOfSimulation, struct Parameters params)
{
    ...
    if (logChains)
    {
        auto tr = FEC.back();
        (*logs).width(8);
        (*logs) << currentTime << " Transact " << tr->id << "(S" << tr->stage << ", T=" << tr->leaveTime << ") (initial transact) added to FEC" << endl;
    }
    ...
    while (1)
    {
        ...
        if (obr->leaveTime > _timeOfSimulation)
            dt = _timeOfSimulation - currentTime;

        /* Сбор статистики */
        stats->QMAX[0] = max(stats->QMAX[0], (int)Q1.size());
        stats->QMAX[1] = max(stats->QMAX[1], (int)Q2.size());
        stats->QAVECONT[0] += Q1.size() * dt;
        stats->QAVECONT[1] += Q2.size() * dt;
        for (int i = 0; i < 3; i++)
        {
            stats->WLOAD[i] += dt * (params.Workers[i] - freeWorkers[i]);
        }
        ...
        {
            (*logs).width(8);
            (*logs) << currentTime << " Transact " << obr->id << "(S" << obr->stage << ") moved to CEC" << endl;
        }
        obr->locked = false;
        obr->eventCallback();
        processCEC();
    ...
    for (int i = 0; i < 3; i++)
    {
        stats->WLOAD[i] /= (double)params.Workers[i] * _timeOfSimulation;
        stats->WLOAD[i] *= 100;
    }
    if (logStats)
        (*logs) << *stats << endl;
...
void System::collectStats()
{
    ...
    for (int i = 0; i < 2; i++)
    {
        stats->QAVECONT[i] /= currentTime;
        stats->QAVETIME[i] /= stats->QENTRY[i];
        stats->QAVETIME0[i] /= stats->QENTRY[i] - stats->QENTRY0[i];
    }
    for (int i = 0; i < 3; i++)
    {
        stats->WAVETIME[i] /= stats->WENTRY[i];
    }
}
    ...
    while (CEC_active_ptr != CEC.end())
    {
        ...
        if (!(tract->locked))
        {
            ...
            if (tract->stage == 1)
            {
                ...
                {
                    (*logs).width(8);
                    (*logs) << currentTime << " Transact " << tract->id << "(S" << tract->stage << ", T=" << tract->leaveTime << ") locked in CEC (Queue1) and added to delay chain" << endl;
                }
            }
            else if (tract->stage == 2)
            {
                ...
                {
                    (*logs).width(8);
                    (*logs) << currentTime << " Transact " << tract->id << "(S" << tract->stage << ", T=" << tract->leaveTime << ") locked in CEC (Queue2) and added to delay chain" << endl;
                }
            }
            else
...
void System::assignTransact2Worker(int type, Transact *tr, double time)
{
    removeFromCEC(tr);
    ...
    if (tr->stage == 1)
    {
        ...
        if (tr->birthTime == tr->startTime[0])
            ...
            stats->QAVETIME0[0] += tr->startTime[0] - tr->birthTime;
    }
    if (tr->stage == 2)
    {
        ...
        if (tr->finishTime[0] == tr->startTime[1])
            ...
            stats->QAVETIME0[1] += tr->startTime[1] - tr->finishTime[0];
    }
    ...
    {
        logs->width(8);
        (*logs) << currentTime << " Transact " << tr->id << "(S" << tr->stage;
        (*logs) << ", T=" << tr->leaveTime << ") moved to FEC by worker type " << type << endl;
    }
    freeWorkers[type - 1]--;
}
...
void System::assignJobs()
{
    while (Q1.size() && freeWorkers[0])
    {
        ...
        Q1.pop();
        assignTransact2Worker(1, tr, exponential_distribution<double>(1 / (double)(R1 + G1 + B1))(rng));
    }
    while (Q2.size() && freeWorkers[1])
    {
        Transact *tr = Q2.front();
        Q2.pop();
        assignTransact2Worker(2, tr, exponential_distribution<double>(1 / (double)(R2 + G2 + B2))(rng));
    }
    while ((Q1.size() || Q2.size()) && freeWorkers[2])
    {
        if (Q1.size() >= Q2.size())
        {
            Transact *tr = Q1.front();
            Q1.pop();
            assignTransact2Worker(3, tr, exponential_distribution<double>(1 / (double)(R3 + G3 + B3 + G1))(rng));
        }
        else
        {
            Transact *tr = Q2.front();
            Q2.pop();
            assignTransact2Worker(3, tr, exponential_distribution<double>(1 / (double)(R3 + G3 + B3 + B1))(rng));
        }
    }
}
...
void Transact::eventCallback()
{
    if (stage == 0)
    {
        ...
        {
            sys->logs->width(8);
            *(sys->logs) << sys->currentTime << " Transact " << tr->id << "(S0, T=" << tr->leaveTime << ") added to FEC" << endl;
        }
    }
    else
    { // Собрать статистику
        ...
        sys->stats->WAVETIME[myW - 1] += finishTime[stage - 1] - startTime[stage - 1];
        ...
        if (stage == 2)
        {
            ...
            {
                sys->logs->width(8);
                *(sys->logs) << sys->currentTime << " Transact " << id << " removed from all chains and terminated" << endl;
            }
        }
        if (stage == 1)
        {
